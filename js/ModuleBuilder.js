function ModuleBuilder(scene,targetList){
    ...
    this.cabinet_w = 30;
    this.rowDistance = 80;
    this.serverDistance = 5;
    this.buildCabinetModule = function () {
        //机柜宽、高、深
        var cabinetModel = {};
        const cabinet_w = 30;
        const cabinet_h = 70;
    ...
    this.buildCabinets = function (rowNum,columnNum,callback) {
        ...
        const cabinet_h = 70;
        const cabinet_d = 30;
        const cabinetModel = this.buildCabinetModule();
        for (let i=0; i<rowNum; i++){
            for (let j=0; j<columnNum; j++){
                let cabinet = {};
                let location = this.convertLocationFromRowNumAndColumnNum(i,j,0);
    ...
    this.convertLocationFromRowNumAndColumnNum = function (rowNum,columnNum,height){
        let object = {};
        object.x = this.initX + columnNum * this.cabinet_w;
        object.y = height ;
        object.z = this.initZ - rowNum * this.rowDistance;
        return object;
    }
    ...
    function openCabinet(o) {
        console.log('openCabinet:' + o.name );
    }

    function closeCabinet(o) {
        $("#serverForm").hide();
        console.log('closeCabinet:' + o.name);
    }


    //服务器宽、高、深
    this.buildServerModel = function (server_w, server_h, server_d) {
        this.server_texture = new THREE.ImageUtils.loadTexture('../images/server.png');
        this.server_texture.wrapS = this.server_texture.wrapT = THREE.ClampToEdgeWrapping;
        this.server_texture.repeat.set(1, 1);
        this.server_texture.minFilter = THREE.LinearFilter

        this.server_back_texture = new THREE.ImageUtils.loadTexture('../images/server-back.png');
        this.server_back_texture.wrapS = this.server_back_texture.wrapT = THREE.ClampToEdgeWrapping;
        this.server_back_texture.repeat.set(1, 1);
        this.server_back_texture.minFilter = THREE.LinearFilter
        ...
        materialArray.push(new THREE.MeshBasicMaterial({color: 0xbbbbbb}));
        materialArray.push(new THREE.MeshBasicMaterial({color: 0xbbbbbb}));
        materialArray.push(new THREE.MeshBasicMaterial({map: this.server_texture}));
        materialArray.push(new THREE.MeshBasicMaterial({map: this.server_back_texture}));
    ...
    this.buildServers = function (serverData,callback) {
        const server_w = 26;
        const server_h = 4;
        const server_d = 26;
        let serverList = [];
        let serverModel = this.buildServerModel(server_w, server_h, server_d);
        ...
        for (let i = 0; i <serverData.length; i++) {
            ...
            let position = this.convertLocationFromRowNumAndColumnNum(rowNum,columnNum,height);
            ...
            server.toggle = function (o, openOrClose) {
                ...
                for (let i = 0; i < serverList.length; i++) {
                    if (serverList[i].position.z - server_d / 2 == o.position.z) {
                        serverList[i].position.z = o.position.z - server_d / 2;
                        //
